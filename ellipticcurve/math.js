...
var multiplyAndAdd = function (p1, n1, p2, n2, N, A, P) {
...
var fromJacobian = function (p, P) {
    ...
    var z = inv(p.z, P);
    var z2 = modulo(z.multiply(z), P);
    var z3 = modulo(z2.multiply(z), P);
...
var shamirMultiply = function (jp1, n1, jp2, n2, N, A, P) {
    // Compute n1*p1 + n2*p2 using Shamir's trick (simultaneous double-and-add).
    // Not constant-time — use only with public scalars (e.g. verification).
    ...
    let jp1p2 = jacobianAdd(jp1, jp2, A, P);

    let l = Math.max(n1.bitLength().toJSNumber(), n2.bitLength().toJSNumber());
